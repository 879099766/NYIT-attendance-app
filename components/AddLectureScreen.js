...
function AddLecture(props) {
  ...
  // Hook for displaying first date from date wheel
  const [fDate, setfDate] = useState("Date")

  // Hook for displaying start time from time wheel
  const [sTime, setsTime] = useState("Time")

  
  // Enable pick date & time
...
const handleDateConfirm = (date) => {
  ...
  setfDate(moment(date).format("MMM Do YY"));
  console.log(date);  
}

const handleTimeConfirm = (time) => {
  hideTimePicker();
  setsTime(moment(time).format('LT'));
  console.log(time)
}
